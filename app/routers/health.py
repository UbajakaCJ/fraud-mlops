...
async def _check_mongodb():
    ...
    try:
        ...
        return DependencyHealth(
            name="mongodb", status=HealthStatus.DOWN, latency_ms=None, detail=str(e)
        )
...
async def _check_redis():
    ...
    try:
        ...
        return DependencyHealth(
            name="redis", status=HealthStatus.DEGRADED, latency_ms=None, detail=str(e)
        )
...
async def _check_mlflow():
    ...
    try:
        ...
        return DependencyHealth(
            name="mlflow",
            status=HealthStatus.OK if resp.status_code == 200 else HealthStatus.DEGRADED,
            latency_ms=round((time.perf_counter() - t0) * 1000, 2),
            detail=None,
        )
    except Exception as e:
        return DependencyHealth(
            name="mlflow", status=HealthStatus.DEGRADED, latency_ms=None, detail=str(e)
        )
...
@router.get("/health", response_model=HealthResponse, summary="Full dependency health check",
            tags=["health"])
async def health():
    """Comprehensive health check: API, model, MongoDB, Redis, MLflow."""
    deps = [await _check_mongodb(), await _check_redis(), await _check_mlflow()]
